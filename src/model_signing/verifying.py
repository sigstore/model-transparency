...
class Config:
    ...
    def verify(
        ...
        signature: hashing.PathLike | str | bytes,
    ):
        ...
        if self._uses_sigstore:
            sig = sigstore.Signature.read(signature)
        else:
            sig = sigstore_pb.Signature.read(signature)

        expected_manifest = self._verifier.verify(sig)
    ...
    def use_elliptic_key_verifier(
        self, *, public_key: hashing.PathLike | bytes
    ) -> Self:
