...
class Signer(sigstore_pb.Signer):
    ...
    def __init__(self, pkcs11_uri: str, module_paths: list[str] = MODULE_PATHS):
        ...
        self._private_key = self.find_object(
            pkcs11.ObjectClass.PRIVATE_KEY, label=label, id=keyid
        )
        public_key = self.find_object(
            pkcs11.ObjectClass.PUBLIC_KEY, label=label, id=keyid
        )
    ...
    def find_object(
        ...
        for obj in self.session.get_objects({pkcs11.Attribute.CLASS: clas}):
            if label is not None and label != obj.label:
                continue
            if id is not None and id != obj.id:
                continue
            return obj
    ...
    def sign(self, payload: signing.Payload) -> signing.Signature:
        ...
        hash_alg = get_ec_key_hash(self._public_key)

        hash = hashes.Hash(hash_alg)
        hash.update(sigstore_pb.pae(raw_payload))
        digest = hash.finalize()

        rs_sig = self._private_key.sign(digest, mechanism=Mechanism.ECDSA)
        sig = encode_ecdsa_signature(rs_sig)
...
class CertSigner(Signer):
    ...
    def __init__(
        ...
        module_paths: list[str] = MODULE_PATHS,
    ):
