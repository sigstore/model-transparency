...
class Pkcs11URI:
    ...
    def validate(self) -> None:
        ...
        if v is not None and not os.path.isabs(v):
            raise ValueError(
                f"path {v} of module-name attribute must be absolute"
            )
    ...
    def set_module_directories(self, dirs: list[str]) -> None:
    ...
    def open_session(
        ...
        for token in lib.get_tokens():
            if token_label is not None and token.label != token_label:
                continue
            if token.slot.slot_id == slot_id:
                self.token = token
                return self.token.open(user_pin=pin)
        raise ValueError(f"Could not find a slot with id {slot_id}")
    ...
    def login(self) -> tuple[pkcs11.Session, pkcs11.lib]:
        ...
        if token_label is None:
            raise pkcs11.ArgumentsBad("Need a token due to missing slot-id")

        for slot in lib.get_slots():
            token = slot.get_token()
            if token.label == token_label:
                self.token = token
                return self.token.open(user_pin=pin), lib
