...
def verify_uri(uri: Pkcs11URI, expecteduri: str) -> None:
    encoded = uri.format()
    assert encoded == expecteduri


def verify_pin(uri: Pkcs11URI, expectedpin: str) -> None:
    assert uri.has_pin()
    pin = uri.get_pin()
    assert pin == expectedpin


def _test_construct(uri: Pkcs11URI, expecteduri: str) -> None:
    verify_uri(uri, expecteduri)
    ...
    verify_uri(uri, expecteduri)
    verify_pin(uri, expectedpin)

    uri.remove_query_attribute("pin-value")
    uri.add_query_attribute_unencoded("pin-value", expectedpin.encode("utf-8"))

    verify_uri(uri, expecteduri)
    verify_pin(uri, expectedpin)


def test_construct1() -> None:
    uri = Pkcs11URI()

    uri.add_path_attribute("id", "%66oo")
    _test_construct(uri, "pkcs11:id=%66%6F%6F")


def test_construct2() -> None:
    uri = Pkcs11URI()
...
def test_construct3() -> None:
    uri = Pkcs11URI()

    uri.add_path_attribute("slot-id", "12345")
    _test_construct(uri, "pkcs11:slot-id=12345")


def test_construct4() -> None:
    uri = Pkcs11URI()
...
def test_pin_source(tmp_path: pathlib.Path) -> None:
    uri = Pkcs11URI()
...
def test_bad_input() -> None:
    ...
    for entry in [
        ...
        ("library-version", "foo", "Invalid format for library-version 'foo'"),
        (
            "library-version",
            "1.bar",
...
def test_validate_escaped_attrs() -> None:
    uri = Pkcs11URI()
...
def test_get_module_restricted() -> None:
    uri = Pkcs11URI()
    uri.set_module_directories(MODULE_PATHS)
...
def run_softhsm_setup(cmd: str) -> Optional[bytes]:
...
def test_softhsm(tmp_path: pathlib.Path) -> None:
    if shutil.which("softhsm2-util") is None:
        return

    stdout_b = run_softhsm_setup("setup")
    if stdout_b is None:
        return
    ...
    try:
        ...
        stdout = run_softhsm_setup("getpubkey")
        ...
        model_signing.signing.Config().use_pkcs11_signer(
            ...
            model_signing.hashing.Config().set_ignored_paths(paths=[signature])
