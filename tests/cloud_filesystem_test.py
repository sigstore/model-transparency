...
def test_remote_directory_marker_is_not_recursed(
    fake_gcs: _FakeGcsFileSystem,
) -> None:
    """An explicit object-store directory marker cannot recurse into itself."""
    fake_gcs.pipe("gs://bucket/model/nested/", b"")

    paths = list(_filesystem.walk_paths(epath.Path("gs://bucket/model")))

    assert [str(path) for path in paths] == [
        "gs://bucket/model",
        "gs://bucket/model/nested",
    ]
...
def test_sign_and_verify_remote_model(
    ...
    sharded_signature_path = tmp_path / "model-sharded.sig"
    signing.Config().use_elliptic_key_signer(
        private_key=private_key_path
    ).set_hashing_config(
        hashing.Config().use_shard_serialization(shard_size=4)
    ).sign("gs://bucket/model", sharded_signature_path)
    verifying.Config().use_elliptic_key_verifier(
        public_key=public_key_path
    ).set_hashing_config(
        hashing.Config().use_shard_serialization(shard_size=4)
    ).set_ignore_unsigned_files(True).verify(
        "gs://bucket/model", sharded_signature_path
    )
...
def test_local_epath_preserves_symlink_policy(tmp_path: pathlib.Path) -> None:
    """A scheme-less epath cannot bypass the local symlink policy."""
    ...
    with pytest.raises(ValueError, match="symlink"):
        hashing.Config().use_file_serialization().hash(epath.Path(model))


@pytest.mark.parametrize("use_shards", [False, True])
def test_selected_file_rejects_symlinked_parent(
    use_shards: bool, tmp_path: pathlib.Path
) -> None:
    """Selective hashing cannot follow an ancestor symlink outside the model."""
    model = tmp_path / "model"
    model.mkdir()
    outside = tmp_path / "outside"
    outside.mkdir()
    (outside / "weights.bin").write_bytes(b"weights")
    (model / "nested").symlink_to(outside, target_is_directory=True)
    config = hashing.Config()
    if use_shards:
        config.use_shard_serialization(shard_size=4)
    else:
        config.use_file_serialization()

    with pytest.raises(ValueError, match="symlink"):
        config.hash(model, files_to_hash=[model / "nested" / "weights.bin"])


@pytest.mark.parametrize("identifier", ["../outside.bin", "/outside.bin"])
def test_signed_identifier_cannot_escape_model_root(
    identifier: str, tmp_path: pathlib.Path
) -> None:
    """Signed resource names are validated before joining the model root."""
    with pytest.raises(ValueError, match="relative to the model root"):
        _filesystem.relative_path_parts(identifier)
...
def test_windows_unsafe_signed_identifier_is_rejected(identifier: str) -> None:
    """Windows-local verification rejects special path components."""
    with pytest.raises(ValueError, match="safe Windows model path"):
        _filesystem.relative_path_parts(identifier, windows_compatible=True)


def test_posix_colon_filename_remains_supported(tmp_path: pathlib.Path) -> None:
    """A colon remains a valid filename outside Windows-local verification."""
    model = tmp_path / "model"
    model.mkdir()
    (model / "weights:v1").write_bytes(b"weights")

    result = hashing.Config().use_file_serialization().hash(model)

    assert [rd.identifier for rd in result.resource_descriptors()] == [
        "weights:v1"
    ]
